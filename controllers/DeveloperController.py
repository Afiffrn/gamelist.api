...
def get_developers():
    ...
    developers = Developer.query.order_by(Developer.nama_dev.asc()).all()
    developer_list = []
    for developer in developers:
        developer_list.append({
            'id_dev': developer.id_dev,
            'nama_dev': developer.nama_dev,
        })

    return jsonify({'status': 'success', 'data': {'developers': developer_list}}), 200
